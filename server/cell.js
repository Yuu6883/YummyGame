    ...
    putHead(pid){
        ...
        if (this.isEmpty() || this.background_pid) {
            ...
            if (this.head_pid === pid || this.isWall) {
                ...
                if (this.background_pid === pid){
                    // Player's territory
                    this.game.emit("kill", {by: pid, killed: this.head_pid});
                } else if (this.background_pid === this.head_pid) {
                    // Other's territory
                    this.game.emit("kill", {by: this.head_pid, killed: pid});
                } else {
        ...
        if (this.tail_pid) {
            if (this.tail_pid == pid) {
                // Player's own tail
                this.game.emit("kill", {by: pid, killed: pid});
            } else {
                this.game.emit("kill", {by: pid, killed: this.tail_pid});
                this.head_pid = pid;
            }
        }
    }
